...
// IMPORT FUNCTIONS
import {
  checkingPictureFormat,
  checkingUserData,
} from "../../utils/checkingUserData";
import { createUser } from "../../utils/fetchUser";

const SignUpContainer = ({ setMessage, setMenu }) => {
  ...
  const addFileHandle = async (e) => {
    const controlPicture = await checkingPictureFormat(e.target.files[0]);
    if (controlPicture === "VALID") {
      setAvatar(URL.createObjectURL(e.target.files[0]));
      setAvatarFile(e.target.files[0]);
      setAvatarChange(true);
    } else {
      setMessage(controlPicture);
    }
  };
